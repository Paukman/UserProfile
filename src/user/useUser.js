...
import { formatBirtday, invertColor, swapCurrentAndPreviousState } from "./utils";
...
const useUser = () => {
  ...
  useEffect(()=> {
      ...
      const isBirthdayToday = ((today.getDate() === userState.currentState.birthday.getDate()) &&
  ...
  const undoChanges = () => {
      const { currentState, previousState } = 
        swapCurrentAndPreviousState(userState.currentState, userState.previousState);
        setState(state => ({
            ...state,
            currentState,
            previousState
